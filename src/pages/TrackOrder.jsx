  ...
  return (
    <div style={{ backgroundColor: '#f8faf8', borderRadius: '14px', padding: '1.25rem 1.5rem', border: '1px solid #edf2ed' }}>
      ...
      {so.items && so.items.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem', padding: '0.75rem', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #edf2ed' }}>
          {so.items.map((item, idx) => (
            <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.5rem', fontSize: '0.75rem' }}>
              <div style={{ flex: 1 }}>
                <p style={{ margin: 0, fontWeight: 600, color: '#1b2d2a' }}>{item.product_name}</p>
                <p style={{ margin: '0.1rem 0 0', fontSize: '0.65rem', color: '#94a3b8' }}>{item.variant_name}</p>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                <p style={{ margin: 0, fontWeight: 700, color: '#1b2d2a' }}>×{item.quantity}</p>
                <p style={{ margin: '0.1rem 0 0', color: '#64748b' }}>₹{(parseFloat(item.unit_price) * item.quantity).toLocaleString('en-IN')}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* AWB & Courier info */}
      {so.awb_number && (
        <div style={{ marginBottom: '0.75rem', padding: '0.75rem', backgroundColor: 'white', borderRadius: '8px', border: '1px solid #d1fae5' }}>
          <p style={{ margin: '0 0 0.3rem', fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>
            AWB Number
          </p>
          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 700, color: '#1b2d2a', fontFamily: 'monospace' }}>
            {so.awb_number}
          </p>
          {so.courier_name && (
    ...
    <div className="container" style={{ padding: '4rem 1rem 10rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '3rem', marginBottom: '4rem' }}>
        <section style={{ backgroundColor: 'white', padding: '3rem', borderRadius: '32px', border: '1px solid #f1f5f9' }}>
          <h1 style={{ fontSize: '2.5rem', fontFamily: 'var(--font-serif)', marginBottom: '1rem' }}>
            Track Your Order
          </h1>
          ...
          <form onSubmit={handleSearch} style={{ display: 'flex', gap: '1rem', flexDirection: window.innerWidth <= 640 ? 'column' : 'row' }}>
            ...
            <button
              ...
              disabled={loading}
              style={{
                padding: window.innerWidth <= 640 ? '1rem 1.5rem' : '1rem 2.5rem',
          ...
          {error && searched && (
            <div style={{ marginTop: '1.5rem', padding: '1rem', backgroundColor: '#fee2e2', border: '1px solid #fca5a5', borderRadius: '12px', color: '#991b1b', fontSize: '0.9rem' }}>
              {error}
            </div>
          )}
        </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth <= 768 ? '1fr' : '1.5fr 1fr', gap: window.innerWidth <= 768 ? '2rem' : '4rem', alignItems: 'start' }}>
          ...
          <div className="slide-up">
            <section style={{ backgroundColor: 'white', padding: '3.5rem', borderRadius: '32px', border: '1px solid #f1f5f9', marginBottom: '3rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4rem' }}>
                <div>
                  ...
                  <p style={{ color: '#64748b', fontWeight: 600 }}>Reference: #{orderData.order_number}</p>
                </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', position: 'relative', marginBottom: '6rem' }}>
                <div style={{ position: 'absolute', top: '24px', left: '0', right: '0', height: '2px', backgroundColor: '#f1f5f9', zIndex: 0 }} />
                {steps.map((step, idx) => (
                  <div key={idx} style={{ position: 'relative', zIndex: 1, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                    ...
                    <span style={{ fontSize: '0.7rem', fontWeight: 800, textTransform: 'uppercase', color: step.active ? 'var(--bg-deep)' : '#94a3b8' }}>
                      {step.label}
                    </span>
                  </div>
                ))}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth <= 640 ? '1fr' : '1fr 1fr', gap: '2rem', padding: '2rem 0', borderTop: '1px solid #f1f5f9' }}>
                <div>
                  <p style={{ margin: '0 0 0.5rem', fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>
                    Total Items
                  </p>
                  <p style={{ margin: 0, fontSize: '1.5rem', fontWeight: 800, color: '#1b2d2a' }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    ...
                    />
                    <span style={{ fontSize: '0.9rem', fontWeight: 700, color: '#1b2d2a' }}>
            ...
            {orderData.items && orderData.items.length > 0 && (
              <section style={{ backgroundColor: 'white', padding: '2rem', borderRadius: '24px', border: '1px solid #f1f5f9', marginBottom: '3rem' }}>
                <h3 style={{ fontSize: '1.2rem', fontFamily: 'var(--font-serif)', marginBottom: '1.5rem' }}>
                  Order Items ({orderData.items.length})
                </h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {orderData.items.map((item) => (
                    <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', backgroundColor: '#f8faf8', borderRadius: '12px', border: '1px solid #edf2ed' }}>
                      {item.product_image && (
                        <img
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ margin: 0, fontWeight: 600, fontSize: '0.9rem', color: '#1b2d2a' }}>
                          {item.product_name}
                        </p>
          ...
          <div className="slide-up" style={{ animationDelay: '0.1s' }}>
            <section style={{ backgroundColor: '#f8faf8', padding: '2.5rem', borderRadius: '24px', border: '1px solid #edf2ed', position: window.innerWidth <= 768 ? 'relative' : 'sticky', top: window.innerWidth <= 768 ? 'auto' : '100px' }}>
              <h4 style={{ fontSize: '1rem', fontFamily: 'var(--font-serif)', marginBottom: '1.5rem', color: '#1b2d2a' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <div style={{ padding: '1rem', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #edf2ed' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: '#1b2d2a', fontFamily: 'monospace' }}>
                ...
                <div style={{ padding: '1rem', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #edf2ed' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '0.95rem', fontWeight: 600, color: '#1b2d2a' }}>
                ...
                <div style={{ padding: '1rem', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #edf2ed' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem' }}>
                    ...
                    <span style={{ fontSize: '0.9rem', fontWeight: 700, textTransform: 'capitalize', color: '#1b2d2a' }}>
                ...
                <div style={{ padding: '1rem', backgroundColor: 'white', borderRadius: '12px', border: '1px solid #edf2ed' }}>
                  ...
                  <p style={{ margin: 0, fontSize: '1.2rem', fontWeight: 800, color: '#1b2d2a' }}>
      ...
      {searched && !orderData && !loading && (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', backgroundColor: 'white', borderRadius: '32px', border: '1px solid #f1f5f9' }}>
          ...
          <h3 style={{ fontSize: '1.3rem', fontWeight: 700, marginBottom: '1rem', color: '#1b2d2a' }}>
            No Results Found
          </h3>
